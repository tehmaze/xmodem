...
class XMODEM(object):
    ...
    def send_bytes(self, data_src: bytes | bytearray, retry=16, timeout=60, quiet=False, callback=None):
        duck_stream = DuckStream(data=data_src)
        return self.send(duck_stream, retry=retry, timeout=timeout, quiet=quiet, callback=callback)
    ...
    def recv_bytes(self, data_buffer: bytearray, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0, callback=None):
        data_buffer.clear()
        duck_stream = DuckStream(data_buffer)
        return self.recv(duck_stream, crc_mode=crc_mode, retry=retry, timeout=timeout, delay=delay, quiet=quiet, callback=callback)
